...
class TestRackSslEnforcer < Test::Unit::TestCase
  ...
  context 'that has a regex pattern as :url option' do
    setup { mock_app :url => /www\.example\.org\/admin/ }

    should 'respond with a ssl redirect for www.example.org/admin path' do
      get 'http://www.example.org/admin'
      assert_equal 301, last_response.status
      assert_equal 'https://www.example.org/admin', last_response.location
    end

    should 'respond not redirect ssl requests without matching path' do
      get 'http://www.example.org/foo'
      assert_equal 200, last_response.status
      assert_equal 'Hello world!', last_response.body
    end
    
    should 'respond not redirect ssl requests without matching host' do
  ...
  context 'that has a string path as :url option' do
    setup { mock_app :url => "http://www.example.org/login" }
